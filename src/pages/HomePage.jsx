  ...
  return (
    <div style={{ fontFamily:"'Poppins',sans-serif" }}>
      <style>{`
        ...
        .hp-hero-overlay { position:absolute; inset:0; background:linear-gradient(to right,rgba(0,0,0,0.75) 50%,transparent); display:flex; align-items:center; }
        .hp-hero-content { padding:0 60px; max-width:620px; }
        .hp-hero-title { font-size:42px; font-weight:800; color:#fff; line-height:1.2; margin-bottom:16px; font-family:'Poppins',sans-serif; }
        .hp-hero-desc { font-size:15px; color:rgba(255,255,255,0.85); line-height:1.7; margin-bottom:28px; font-family:'Poppins',sans-serif; }
        .hp-hero-btns { display:flex; gap:14px; flex-wrap:wrap; }
        .hp-btn-red { padding:13px 26px; background:#e05c5c; color:#fff; border:none; border-radius:8px; font-size:14px; font-weight:600; cursor:pointer; font-family:'Poppins',sans-serif; }
        .hp-btn-outline { padding:13px 26px; background:transparent; color:#fff; border:2px solid #fff; border-radius:8px; font-size:14px; font-weight:600; cursor:pointer; font-family:'Poppins',sans-serif; }
        .hp-dots { position:absolute; bottom:20px; left:60px; display:flex; gap:8px; }
        .hp-dot { width:10px; height:10px; border-radius:50%; background:rgba(255,255,255,0.5); cursor:pointer; transition:all 0.3s; }
        .hp-dot-active { background:#fff; width:28px; border-radius:5px; }

        /* Book */
        .hp-book { display:flex; align-items:center; padding:80px 60px; gap:60px; background:#f8fafc; }
        .hp-book-left { flex:1; }
        .hp-appt-slider { position:relative; height:380px; border-radius:16px; overflow:hidden; }
        .hp-appt-img { position:absolute; inset:0; width:100%; height:100%; object-fit:cover; }
        .hp-appt-dots { position:absolute; bottom:14px; left:50%; transform:translateX(-50%); display:flex; gap:8px; z-index:10; }
        .hp-book-right { flex:1; }
        .hp-badge { background:#e8f4fb; color:#2193b0; padding:6px 16px; border-radius:20px; font-size:13px; font-weight:500; font-family:'Poppins',sans-serif; display:inline-block; }
        .hp-section-title { font-size:30px; font-weight:700; color:#1a1a2e; margin:14px 0 10px; font-family:'Poppins',sans-serif; }
        .hp-section-desc { font-size:15px; color:#666; line-height:1.7; font-family:'Poppins',sans-serif; }
        .hp-book-card { background:#fff; border-radius:12px; padding:24px; box-shadow:0 4px 16px rgba(0,0,0,0.08); margin-top:24px; }
        .hp-book-now-btn { width:100%; padding:14px; background:#2193b0; color:#fff; border:none; border-radius:8px; font-size:15px; font-weight:600; cursor:pointer; font-family:'Poppins',sans-serif; margin-bottom:10px; }
        .hp-book-note { text-align:center; font-size:13px; color:#888; font-family:'Poppins',sans-serif; }

        /* Departments */
        .hp-depts { padding:80px 60px; background:#eef6fb; }
        .hp-center-badge { text-align:center; margin-bottom:14px; }
        .hp-depts-grid { display:flex; gap:24px; margin-top:36px; flex-wrap:wrap; }
        .hp-dept-card { flex:1; min-width:240px; background:#fff; border-radius:12px; padding:28px; box-shadow:0 2px 12px rgba(0,0,0,0.06); transition:transform 0.3s,box-shadow 0.3s; }
        .hp-dept-card:hover { transform:translateY(-6px); box-shadow:0 12px 30px rgba(0,0,0,0.12); }
        .hp-dept-name { font-size:18px; font-weight:600; color:#1a1a2e; margin-bottom:12px; font-family:'Poppins',sans-serif; }
        .hp-dept-desc { font-size:14px; color:#666; line-height:1.7; margin-bottom:20px; font-family:'Poppins',sans-serif; }
        .hp-learn-more { color:#2193b0; font-size:14px; font-weight:600; cursor:pointer; font-family:'Poppins',sans-serif; }
        .hp-view-all-wrap { text-align:center; margin-top:28px; }
        .hp-view-all-btn { padding:12px 28px; background:transparent; color:#2193b0; border:2px solid #2193b0; border-radius:8px; font-size:14px; font-weight:600; cursor:pointer; font-family:'Poppins',sans-serif; }

        /* Why */
        .hp-why { padding:80px 60px; background:#fff; }
        .hp-why-title { font-size:34px; font-weight:800; color:#1a1a2e; text-align:center; margin-bottom:10px; font-family:'Poppins',sans-serif; }
        .hp-why-grid { display:flex; gap:22px; flex-wrap:wrap; margin-top:36px; }
        .hp-why-card { flex:1; min-width:200px; border:1px solid #eee; border-radius:12px; padding:28px; text-align:center; transition:border-color 0.2s,transform 0.2s; }
        .hp-why-card:hover { border-color:#2193b0; transform:translateY(-4px); }
        .hp-why-icon { display:flex; justify-content:center; margin-bottom:14px; }
        .hp-why-card-title { font-size:16px; font-weight:600; color:#1a1a2e; margin-bottom:10px; font-family:'Poppins',sans-serif; }
        .hp-why-card-desc { font-size:13px; color:#666; line-height:1.7; font-family:'Poppins',sans-serif; }

        /* Testimonials */
        .hp-testi { padding:80px 60px; background:#f8fafc; }
        .hp-testi-grid { display:flex; gap:24px; flex-wrap:wrap; margin-top:36px; }
        .hp-testi-card { flex:1; min-width:240px; background:#fff; border-radius:12px; padding:28px; box-shadow:0 2px 12px rgba(0,0,0,0.06); transition:transform 0.3s,box-shadow 0.3s; }
        .hp-testi-card:hover { transform:translateY(-6px); box-shadow:0 12px 30px rgba(0,0,0,0.1); }
        .hp-quote { font-size:40px; color:#2193b0; font-weight:800; line-height:1; margin-bottom:12px; }
        .hp-testi-text { font-size:14px; color:#555; line-height:1.8; font-family:'Poppins',sans-serif; }

        /* Contact */
        .hp-contact { display:flex; padding:80px 60px; gap:60px; align-items:flex-start; background:#eef6fb; }
        .hp-contact-left { flex:1; }
        .hp-contact-right { flex:1; }
        .hp-contact-title { font-size:30px; font-weight:700; color:#1a1a2e; margin-bottom:12px; font-family:'Poppins',sans-serif; }
        .hp-contact-desc { font-size:15px; color:#666; line-height:1.7; margin-bottom:24px; font-family:'Poppins',sans-serif; }
        .hp-contact-item { display:flex; gap:12px; margin-bottom:14px; font-size:14px; color:#444; font-family:'Poppins',sans-serif; align-items:flex-start; }

        /* Responsive */
        @media (max-width:900px) {
          .hp-hero { height:380px; }
          .hp-hero-content { padding:0 32px; }
          .hp-hero-title { font-size:30px; }
          .hp-dots { left:32px; }
          .hp-book { flex-direction:column; padding:50px 32px; gap:32px; }
          .hp-appt-slider { height:280px; }
          .hp-depts { padding:50px 32px; }
          .hp-why { padding:50px 32px; }
          .hp-testi { padding:50px 32px; }
          .hp-contact { flex-direction:column; padding:50px 32px; gap:32px; }
        }
        @media (max-width:600px) {
          ...
          .hp-dots { left:20px; bottom:12px; }
          .hp-book { padding:36px 20px; }
          .hp-appt-slider { height:220px; }
          .hp-section-title { font-size:22px; }
          .hp-depts { padding:36px 20px; }
          .hp-depts-grid { flex-direction:column; }
          .hp-why { padding:36px 20px; }
          .hp-why-title { font-size:22px; }
          .hp-why-grid { flex-direction:column; }
          .hp-testi { padding:36px 20px; }
          .hp-testi-grid { flex-direction:column; }
          .hp-contact { padding:36px 20px; }
      ...
      <div className="hp-contact">
        <div className="hp-contact-left">
          ...
          <div className="hp-contact-item">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#2193b0" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{flexShrink:0,marginTop:2}}><path d="M22 16.92v3a2 2 0 0 1-2.18 2 19.79 19.79 0 0 1-8.63-3.07A19.5 19.5 0 0 1 4.69 12 19.79 19.79 0 0 1 1.65 3.44 2 2 0 0 1 3.62 1h3a2 2 0 0 1 2 1.72c.127.96.361 1.903.7 2.81a2 2 0 0 1-.45 2.11L7.91 8.64a16 16 0 0 0 6 6l.96-.96a2 2 0 0 1 2.11-.45c.907.339 1.85.573 2.81.7A2 2 0 0 1 22 16.92z"/></svg>
            <span>04-2558804</span>
          </div>
          <div className="hp-contact-item">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#2193b0" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{flexShrink:0,marginTop:2}}><path d="M21 10c0 7-9 13-9 13s-9-6-9-13a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg>
            <span>Ground & Mezzanine Floor, Doha Centre, Al Maktoum Road, Deira, Dubai.</span>
          </div>
          <div className="hp-contact-item">
            <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="#2193b0" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" style={{flexShrink:0,marginTop:2}}><circle cx="12" cy="12" r="10"/><polyline points="12 6 12 12 16 14"/></svg>
        ...
        <div className="hp-contact-right">
          <iframe title="map"
            ...
            width="100%" height="320" style={{ border:0, borderRadius:"12px" }} allowFullScreen loading="lazy"/>
        </div>
      </div>
